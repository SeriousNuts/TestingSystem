...
namespace RimarKurshach
{
    public partial class FormTest : Form
    {
        ...
        {
            int count = System.IO.File.ReadAllLines("t.txt").Length;
            button_next.Text = "Следующий вопрос";
            kolvo_voprosov = count / 4;
            praviln_otvetov = 0;
            nepraviln_otvetov = 0;
            load_file();
            radio_checked();
        ...
        {
            int percent;
            if (nepraviln_otvetov != 0)
            {
                percent = (praviln_otvetov / nepraviln_otvetov * 100);
            }
            else
            {
                percent = 100;
            }
            MessageBox.Show ( "Правильных ответов: " + praviln_otvetov.ToString() + "\n" +
                "Неправильных ответов: " + nepraviln_otvetov.ToString() + " процент выполнения: " +  percent + "%");
        }
